...
def main(args):
    ...
    for metric in args.distance_metric:
        for database, queries in list(zip(args.database_folder, args.queries_folder)):
            start_time = datetime.now()

            logger.remove()  # Remove possibly previously existing loggers
            log_dir = Path("/content/drive/MyDrive/VPR_preds/logs") / args.log_dir / start_time.strftime("%Y-%m-%d_%H-%M-%S")     # "My Drive/VPR_preds/" in order to save the logs to my drive
            ...
            faiss_index = faiss.IndexFlatL2(args.descriptors_dimension) if metric == 'L2' else faiss.IndexFlatIP(args.descriptors_dimension)  # 1
            ...
            if args.use_labels:
                ...
                if len(args.recall_values) == 1:
                    with open(f"{args.method}_first_performance_eval.csv", "a", newline="\n") as f:   # 1
                        row = " ".join(f"{metric} {queries} {recalls_str}\n")   # 1
                        f.write(row)
