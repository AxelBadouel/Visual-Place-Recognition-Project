...
def main(args):
    ...
    train_data_csv = (
        args.train_inliers_df_csv[0]
        if isinstance(args.train_inliers_df_csv, list)
        else args.train_inliers_df_csv
    )
    val_data_csv = (
        args.val_inliers_df_csv[0]
    ...
    model.eval()
    with torch.no_grad():
        val_probs = model(x_val_scaled.to(device)).cpu()
    ...
    if os.path.dirname(args.out_dir):
